...
combineTrees <- function(object,
  ...
  .validInput(object, "object", "combineTrees") ### MODIFY
  .validInput(subtree_list, "subtree_list", object) #### CREATE
  .validInput(key, "key", list("combineTrees", object)) #### MODIFY
  ...
  if (length(parameters_to_check) > 0) {
    ...
    buildParentTree_parameters <- buildParentTree_parameters[intersect(names(buildParentTree_parameters),
                                                                       parameters_to_check)]
    # Retrieve parameter values from buildParentTree and subtree pruneTree records
    subtree_pruneTree_parameters <- .retrieveData(subtree_list[[1]],
                                                  key,
                                                  "parameters",
                                                  "pruneTree_parameters")
    ...
    if (identical(buildParentTree_parameters, subtree_pruneTree_parameters[names(buildParentTree_parameters)])) {
      for (s in 2:n_subtrees) {
        subtree_pruneTree_parameters_s <- .retrieveData(subtree_list[[s]],
                                                        key,
                                                        "parameters",
                                                        "pruneTree_parameters")
  ...
  .validInput(feature_set, "feature_set", list("combineTrees", normalization_method)) ### MODIFY
  ...
  .validInput(reduction, "reduction", list("combineTrees", object)) ### MODIFY
  ...
  all_cluster_ids <- all_cluster_ids[rownames(object@misc[[key]]$clusters$P0_tree),]
  ...
  cluster_info <- all_cluster_ids %>% group_by(Subtree_cluster, Parent_cluster) %>% summarise(n = n()) %>% data.frame()
  ...
  for (i in 1:(nrow(cluster_info)-1)) {
    ...
    if (length(permitted_comparisons) > 0) {
      ...
      for (p in permitted_comparisons) {
        ...
        adjacent <- sum(seurat_all@misc[[key]]$graph$P0_graph_nn[cluster_i_cells, p_cells]) +
          sum(seurat_all@misc[[key]]$graph$P0_graph_nn[p_cells, cluster_i_cells])
  ...
  cluster_tree <- object@misc[[key]]$clusters$P0_tree
  ...
  if (!is.null(input_matrix)) {
    ...
    .validInput(batch_correction_method, "batch_correction_method", list(n_modalities, "combineTrees")) #### MODIFY
  ...
  while (complete == FALSE) {
    ...
    for (parent in 1:length(unique_parent_IDs)) {
      print(paste0("parent: ", parent, " at ", Sys.time()))
      ...
      print(paste0("# child clusters: ", n_child_clusters))
        ...
        if (dplyr::n_distinct(cluster_tree[parent_inds, lvl+1]) == 1 & check_identical == TRUE) {
          ...
          if (sum(result_matrix == "merge", na.rm = TRUE) == n_child_clusters*(n_child_clusters - 1)) {
            ...
            for (child in 1:n_child_clusters) {
              print(paste0("check merge: ", child))
            ...
            # For each child
            for (child in 1:n_child_clusters) {
              print(paste0("merge group: ", child))
              child_name <- unique_child_IDs[child]
      ...
      for (cluster_i in new_clusters) {
        ...
        if (length(permitted_comparisons) > 0) {
          ...
          for (p in permitted_comparisons) {
            ...
            adjacent <- sum(seurat_all@misc$CHOIR$graph$P0_graph_nn[cluster_i_cells, p_cells]) +
              sum(seurat_all@misc$CHOIR$graph$P0_graph_nn[p_cells, cluster_i_cells])
            if (adjacent >= min_connections) {
              retained_permitted_comparisons <- c(retained_permitted_comparisons, p)
            }
          }
        ...
        if (length(retained_permitted_comparisons) > 0) {
          ...
          print(nrow(permitted_comparison_df))
        }
      }
